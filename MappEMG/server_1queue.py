...
class RunServer():
    ...
    def __init__(self, server_ip='localhost',
        ...
        ----------
        Connected : bool
            Detect in real time whether the client has been connected with the server.
        sensorkit : string
        ...
        server : Server
            The server to be connected with client.
        connection : dict
            If the client is listening the data from server,
            the connection will be defined by server.client_listening
        """
    ...
    def run_emg_processing(self):
        ...
        while True:
            ...
            if is_working:
                ...
                # Process data
                emg_proc = emg_processing.process_emg(data=emg_tmp,
                                                      frequency=self.device_sampling_rate,
                                                      ma=True)

                # Sends the average of the most recent 100 samples processed
                emg_proc_to_send = np.reshape(
                    np.average(emg_proc[:, -self.size_processing_window:], axis=1),
                    (self.n_electrode, 1))

                data_to_send = {}
                data_to_send["emg_raw_all"] = emg_tmp
                data_to_send["emg_proc"] = emg_proc_to_send
